...
def create_app(test_config=None):
  ...
  def questions_get():
    ...
    current_category = set([question['category'] for question in questions])
    data = {
  ...
  def questions_post():
    try:
      ...
      Question(data['question'], data['answer'], data['category'], data['difficulty']).insert()
    except:
  ...
  def questions_search_post():
    try:
      ...
      raw_questions = Question.query.filter(Question.question.ilike(f'%{search_term}%')).all()
  ...
  def questions_quizzes_post():
    try:
      ...
      if quiz_category['id'] == 0:
        raw_question = Question.query.filter(Question.id.notin_(previous_questions)).first()
      else:
        raw_question = Question.query.filter_by(category=quiz_category['id']).filter(Question.id.notin_(previous_questions)).first()
